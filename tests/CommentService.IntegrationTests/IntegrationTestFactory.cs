...
using CommentService.IntegrationTests.Helpers;
...
namespace CommentService.IntegrationTests
{
    public class IntegrationTestFactory : WebApplicationFactory<Program>, IAsyncLifetime
    {
        ...
        public Mock<IVideoServiceClient> videoServiceMock { get; } = new();
        public Mock<ICourseServiceClient> courseServiceMock { get; } = new();
        public Mock<ICoursePermissionsService> permissionsServiceMock { get; } = new();
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.ConfigureTestServices(services =>
            {
                ...
                var permDescriptor = services.SingleOrDefault(d => d.ServiceType == typeof(ICoursePermissionsService));
                if (permDescriptor != null) services.Remove(permDescriptor);
                services.AddSingleton(permissionsServiceMock.Object);

                var videoClientDescriptor = services.SingleOrDefault(d => d.ServiceType == typeof(IVideoServiceClient));
                if (videoClientDescriptor != null) services.Remove(videoClientDescriptor);
                services.AddSingleton(videoServiceMock.Object);

                var courseClientDescriptor = services.SingleOrDefault(d => d.ServiceType == typeof(ICourseServiceClient));
                if (courseClientDescriptor != null) services.Remove(courseClientDescriptor);
                services.AddSingleton(courseServiceMock.Object);

                var tokenServiceDescriptor = services.SingleOrDefault(d => d.ServiceType == typeof(IIdentityTokenService));
                if (tokenServiceDescriptor != null) services.Remove(tokenServiceDescriptor);
                services.AddScoped<IIdentityTokenService>(_ => new Mock<IIdentityTokenService>().Object);
